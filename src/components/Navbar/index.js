...
import { AppBar, Box, Drawer, IconButton, List, ListItem } from '@mui/material';
...
const primaryMenu = [
  ...
  { id: 2, label: 'Buscar', path: '/', icon: SearchIcon },
  { id: 3, label: 'Sua Biblioteca', path: '/', icon: LabraryIcon }
];
const secondaryMenu = [
  { id: 1, label: 'Criar Playlist', path: '/', icon: AddIcon, background: '#FFF', color: '#000' },
  {
    id: 2,
...
const tercearyMenu = [
  { id: 1, label: 'Cookies', path: '/' },
  { id: 2, label: 'Privacidade', path: '/' }
];
      ...
      <div className={classes.container}>
        ...
        <div>
          {primaryMenu.map((item) => (
            <ListItem key={item.id} button disableGutters>
              <a className={`${classes.listLink} ${classes.link}`} to="/">
                <item.icon />
        ...
        <div className={classes.navListButton}>
          {secondaryMenu.map((item) => (
            <ListItem key={item.id} button disableGutters>
              <a className={`${classes.navButton} ${classes.link}`} to="/">
                <div className={classes.navButtonImage}>
                  <div
                    className={classes.navButtonImageBox}
                    style={{ background: item.background, color: item.color }}
                  >
                    <item.icon />
                  </div>
                </div>
                <span className={classes.listItemText} style={{ fontWeight: 400, color: '#FFF' }}>
                  {item.label}
                </span>
              </a>
            </ListItem>
      ...
      >
        <div className={classes.navList}>
          ...
          <div>
            ...
            <button type="button" className={classes.navButtonWhite}>
              ENTRAR
            </button>
          </div>
        </div>
